...
အဆင်ပြေစွာ အသုံးပြုနိုင်ပါစေ။ ကျေးဇူးတင်ပါသည်။
  ...
  const handleExportPdf = async () => {
    ...
    try {
      await exportReceiptToPdfAndShare(transaction, shopProfile);
    } catch (err) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="grid grid-cols-2 sm:flex sm:flex-row gap-2 pt-2 border-t border-slate-100 shrink-0">
          <button
            onClick={handleCopyText}
            className="py-2.5 px-2 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl text-xs font-bold transition-colors flex items-center justify-center gap-1.5 cursor-pointer"
          >
            {copied ? (
              <>
          ...
          <button
            ...
            className="py-2.5 px-2 bg-rose-600 hover:bg-rose-700 disabled:opacity-50 text-white rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-1.5 shadow-sm cursor-pointer"
            title="PDF ဖိုင်အဖြစ် ပြောင်းပြီး ဖုန်းထဲသိမ်းဆည်း / Native Share လုပ်မည်"
          >
            {isPdfExporting ? (
              <>
          ...
          <button
            ...
            className="py-2.5 px-2 bg-purple-600 hover:bg-purple-700 text-white rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-1.5 shadow-md shadow-purple-600/20 cursor-pointer"
          >
            {isBtPrinting ? (
              <>
          ...
          <button
            onClick={handlePrint}
            className="py-2.5 px-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold transition-colors flex items-center justify-center gap-1.5 shadow-md shadow-indigo-600/20 cursor-pointer"
          >
